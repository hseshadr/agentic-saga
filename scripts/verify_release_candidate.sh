...
"$venv"/bin/python -c \
  'import agentic_saga, temporalio; import agentic_saga.temporal as temporal; from agentic_saga.agents import PydanticAIDriver, OpenRouterSettings, build_openrouter_driver; assert agentic_saga.WorkflowState.__module__ == "agentic_saga.temporal.contracts"; assert callable(agentic_saga.start_saga); assert all(callable(item) for item in (temporal.TemporalActivities, temporal.build_worker, temporal.connect_client, temporal.project_run_trace)); assert not hasattr(agentic_saga, "SagaRuntime")'
