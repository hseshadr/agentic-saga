...
| Objective, resolved public context, current observation, eligible schemas or candidate proposals | Local by default. Sent to OpenRouter or TypeSafe AI only by the explicitly selected optional adapter path. A Jev request includes the full public observation, eligible descriptors, and every complete candidate's criteria, proposal arguments, and rationale. The exact definition-owned policy redacts classified keys before runtime observations or evidence are rendered; manifest context and Jev candidates must already be public. | Host-owned memory and durable evidence. The host chooses lawful retention and deletes its stores/backups. TypeSafe publicly states requests are not used for training; the direct route makes no zero-data-retention assumption. The OpenRouter Decisions route requires provider-routing flags `zdr: true` and `data_collection: deny`, but OpenRouter account logging settings and request-metadata retention are separate controls. Operators must verify and contract for each provider's retention and deletion. |
...
| `TYPESAFE_API_KEY` | Read from the process environment or injected Jev settings; masked and excluded from state, criteria, metadata, and safe exceptions. | Process/configuration lifetime. Rotate and revoke through TypeSafe AI; never store it in `saga.yaml` or a candidate. |
