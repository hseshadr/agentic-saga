...
- Duplication: HTTP status classification is repeated in `agents/deepagents.py:612`, `agents/jev.py:242`, and `agents/openrouter_decisions.py:304`; share classification while retaining provider-specific exception handling.
- Optional migration: `agents/deepagents.py:91` disables most Pydantic Deep capabilities. Direct Pydantic AI could remove that wrapper dependency; prove tool selection, deferred execution, and request limits before changing it. This is a design option, not a current defect. [Deferred tools](https://ai.pydantic.dev/deferred-tools/)
