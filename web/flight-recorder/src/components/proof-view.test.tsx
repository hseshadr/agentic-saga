...
describe("ProofView", () => {
  it("shows exact recorded rule, source, version, result, and target", async () => {
    ...
    const select = vi.fn();
    const replay = projection("happy-path");
    ...
    expect(screen.getByText("verify_order")).toBeVisible();
    expect(screen.getByText("temporal-proof-v1")).toBeVisible();
    expect(screen.getByText("succeeded_verified")).toBeVisible();
  ...
  it("labels compensation as a completion record, never as an invariant proof", async () => {
    const user = userEvent.setup();
    const select = vi.fn();
    const replay = projection("business-failure");
    render(<ProofView onSelectEvent={select} projection={replay} />);

    expect(screen.getByText("3 of 3 undo steps confirmed")).toBeVisible();
    expect(screen.getByText(/completion record, not an invariant proof/)).toBeVisible();
    expect(screen.getByText("0 of 1 recorded invariants valid")).toBeVisible();
    expect(screen.queryByText("obligations_reversed")).not.toBeInTheDocument();
    const record = replay.recovery.record;
    await user.click(
      screen.getByRole("button", { name: `Inspect recovery record event ${record?.saga_seq}` }),
    );
    expect(select).toHaveBeenCalledWith(record?.event_id);
  });
